...
* FIXED: **A rating scale of zero is repaired on update.** The pre-2.0.0 settings screen saved whatever the Max field posted, and saved 0 whenever it posted nothing -- which a custom image set made it do, since it filled that field from the image folder and made it read-only, and a folder since deleted counted no images. A site carrying that zero had no scale for its ratings to be out of: the structured data offered Google `bestRating` of 0, which Search Console reports as an error, and working out the rating percentage divided by it, which is fatal on PHP 8. The scale is now rebuilt from the rating text table, which is what defines it, and a site with no usable table gets the five-point scale the plugin has always installed. A scale you chose is left alone.
