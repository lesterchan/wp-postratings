...
* BREAKING: The shared, unprefixed `stats_display` and `stats_mostlimit` option rows are no longer read. WP-Stats integration is now two settings of this plugin's own, on the Settings tab.
...
* NEW: Rating colours are a setting. Pick the rated and not-rated colours on the Settings tab; this replaces the old colour variants of the image sets, where changing colour meant choosing a different set of files.
