...
function the_ratings($start_tag = 'div', $custom_id = 0, $display = true) {
	...
	if ( (int) $postratings_ajax_style['loading'] === 1 ) {
		...
		$loading = '<' . $start_tag . ' id="post-ratings-' . $ratings_id . '-loading" class="post-ratings-loading"><img src="' . plugins_url('wp-postratings/images/loading.gif') . '" alt="post-ratings-image" width="16" height="16" class="post-ratings-image"' . $loading_alt_html . ' />' . esc_html__( 'Loading...', 'wp-postratings' ) . '</' . $start_tag . '>';
	} else {
		$loading = '';
	}
