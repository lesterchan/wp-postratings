...
function the_ratings($start_tag = 'div', $custom_id = 0, $display = true) {
	...
	if(intval($postratings_ajax_style['loading']) == 1) {
		$loading = '<div id="post-ratings-' . $ratings_id . '-loading"  class="post-ratings-loading">
			<img src="' . plugins_url('wp-postratings/images/loading.gif') . '" width="16" height="16" alt="' . __( 'Loading...', 'wp-postratings' ) . '" title="' . __( 'Loading...', 'wp-postratings' ) . '" class="post-ratings-image" />' . __( 'Loading...', 'wp-postratings' ) . '</div>';
	} else {
		$loading = '';
	}
